...
/* We don't use ifdefs around these since once a packet number is picked
   for something it should never be used again */
...
struct stats_spacket {
   ...
   int            tlosses;	/* Tournament losses */
   int            kills;	/* overall */
   int            losses;	/* overall */
   int            tticks;	/* ticks of tournament play time */
   int            tplanets;	/* Tournament planets */
   int            tarmies;	/* Tournament armies */
   int            sbkills;	/* Starbase kills */
   int            sblosses;	/* Starbase losses */
   int            armies;	/* non-tourn armies */
   int            planets;	/* non-tourn planets */
   int            maxkills;	/* max kills as player * 100 */
   int            sbmaxkills;	/* max kills as sb * 100 */
};
...
struct udp_req_cpacket {	/* UDP */
   ...
   char            request;
   char            connmode;	/* respond with port # or just send UDP
				 * packet? */
   char            pad2;
...
struct ping_cpacket {
   ...
   int            cp_sent;	/* # packets sent to server */
   int            cp_recv;	/* # packets recv from server */
};
...
struct player_s_spacket {
   ...
   char            packets;	/* How many player-packets are in this packet
				 * ( only the first 6 bits are relevant ) */
   unsigned char   dir;
   char            speed;
   int            x, y;	/* To get the absolute Position */
};
/* S_P2 */
...
/*
 * The format of the body: struct player_s_body_spacket {  Body of new Player
 * Packet unsigned char pnum;      0-4 = pnum, 5 local or galactic, 6 = 9.
 * x-bit, 7 9. y-bit unsigned char speeddir;  0-3 = speed , 4-7 direction of
 * ship unsigned char x;         low 8 bits from X-Pixelcoordinate unsigned
 * char y;         low 8 bits from Y-Pixelcoordinate };
 */

struct torp_s_spacket {
   char            type;	/* SP_S_TORP */
   unsigned char   bitset;	/* bit=1 that torp is in packet */
   unsigned char   whichtorps;	/* Torpnumber of first torp / 8 */
   unsigned char   data[21];	/* For every torp 2*9 bit coordinates */
};
...
struct phaser_s_spacket
  {
    ...
    char status;                /* PH_HIT, etc... */
    unsigned char pnum;         /* both bytes are used for more */
...
struct motd_pic_spacket {
    ...
    short   index, line;
    short   x, y;
    /* bits,width,height,size are sent as SP_BITMAP */
};

struct bitmap_spacket {         /* SP_BITMAP */
   char 	type;
   char		bitmap_type;
   char		misc1, misc2;
   int		size;
